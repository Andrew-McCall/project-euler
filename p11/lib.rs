...
pub fn get_point(data: &[u64], width: usize, x: usize, y: usize) -> Point {
    Point {
        x_coordinate: x as u64,
        y_coordinate: y as u64,
        value: get_point_value(data, width, x, y),
    }
}
...
impl std::fmt::Display for Result {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let coords_str = self
            .points
            .iter()
            .map(|p| format!("{} @ ({}, {})", p.value, p.x_coordinate, p.y_coordinate))
            .collect::<Vec<_>>()
            .join(", ");
